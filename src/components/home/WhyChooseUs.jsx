...
import gpuRender from '../../assets/images/gpu_render.png';
import moboRender from '../../assets/images/motherboard_render.png';
import coolerRender from '../../assets/images/cooler_render.png';
import cpuRender from '../../assets/images/cpu_render.png';
import caseRender from '../../assets/images/case_render.png';
...
const CardImage = ({ src, alt }) => {
  ...
  return (
    <div className="overflow-hidden rounded-[16px] bg-[#101010] border border-white/5 mb-6 relative group/img">
      <div className="h-56 w-full overflow-hidden flex items-center justify-center p-4">
        {!imageError ? (
          <motion.img
            src={src}
            alt={alt}
      ...
      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_center,transparent_40%,rgba(16,16,16,0.6)_100%)]" />
    </div>
  );
};
        ...
        {chooseCards.map((card, index) => {
          ...
          return (
            <motion.div
              ...
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              whileHover={{ y: -6, boxShadow: "0 25px 50px -12px rgba(255,255,255,0.03)" }}
              viewport={{ once: true, margin: '-50px' }}
              transition={{ duration: 0.6, delay: index * 0.08, ease: [0.16, 1, 0.3, 1] }}
              className={`${sizeClasses} h-full group`}
            >
              <Card hoverEffect={false} className="flex flex-col justify-between h-full transition-colors duration-500 group-hover:border-white/15">
